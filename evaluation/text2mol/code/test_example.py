...
from dataloaders import (
    ...
    GenerateData,
    get_graph_data,
    get_attention_graph_data,
    GenerateDataAttention,
    get_attention_dataloader,
)
...
if MODEL == "MLP":
    ...
    training_generator, validation_generator, test_generator = get_dataloader(
        gd, params
    )
    ...
    graph_batcher_tr, graph_batcher_val, graph_batcher_test = get_graph_data(
        gd, graph_data_path
    )
    ...
    training_generator, validation_generator, test_generator = get_attention_dataloader(
        gd, params
    )

    graph_batcher_tr, graph_batcher_val, graph_batcher_test = get_attention_graph_data(
        gd, graph_data_path, mol_trunc_length
    )
    ...
    model = AttentionModel(
        ...
        nhid=512,
        nlayers=3,
        graph_hidden_channels=768,
        mol_trunc_length=mol_trunc_length,
        temp=0.07,
    )
...
all_text_embbedings = np.concatenate(
    (text_embeddings_train, text_embeddings_val, text_embeddings_test), axis=0
)
all_mol_embeddings = np.concatenate(
    (chem_embeddings_train, chem_embeddings_val, chem_embeddings_test), axis=0
)
...
def name_to_input(name, data_generator):
    ...
    return {
        ...
        "input": {
            ...
            "molecule": {"mol2vec": torch.zeros((1, 300)), "cid": ""},
        },
    }
