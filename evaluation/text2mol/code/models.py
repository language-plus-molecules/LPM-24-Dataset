...
class MLPModel(nn.Module):
    def __init__(self, ninp, nout, nhid):
        ...
        self.text_transformer_model = BertModel.from_pretrained(
...
class AttentionModel(nn.Module):
    ...
    def forward(self, text, graph_batch, text_mask=None, molecule_mask=None):
        ...
        node_features = torch.zeros(
            (graph_batch.num_graphs, self.mol_trunc_length, self.graph_hidden_channels)
        ).to(self.device)
        for i, p in enumerate(graph_batch.ptr):
            if p == 0:
                old_p = p
                continue
            node_features[i - 1, : p - old_p, :] = mol_x[
