...
def evaluate(
    ot_caps, smis, text_model="allenai/scibert_scivocab_uncased", text_trunc_length=512
):

    outputs = [(smi, proc_line(b)) for b, smi in zip(ot_caps, smis)]
...
def create_scores(zip_file, out_name):
    ...
    props = evaluate(ot_caps, truth_smis)
